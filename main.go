...
func main() {
	...
	tmeTaxonomyName := "Genres"

	app.Action = func() {
		client := getResilientClient()

		mf := new(genreTransformer)
		s, err := newGenreService(tmereader.NewTmeRepository(client, *tmeBaseURL, *username, *password, *token, *maxRecords, *slices, tmeTaxonomyName, &tmereader.KnowledgeBases{}, mf), *baseURL, tmeTaxonomyName, *maxRecords)
		if err != nil {
			log.Errorf("Error while creating GenresService: [%v]", err.Error())
		}
		...
		m.HandleFunc("/__health", v1a.Handler("Genres Transformer Healthchecks", "Checks for accessing TME", h.HealthCheck()))
